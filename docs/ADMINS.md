...
| `TAG_SOURCE_SERVERS` | ten servers | Which servers the **"Where should this tag come from?"** panel offers a member (issue #2128), comma-separated hostnames. The shipped ten are the vutuv.de list — `mastodon.social`, `mastodon.world`, `mastodon.online`, `hachyderm.io`, `troet.cafe`, `social.tchncs.de`, `det.social`, `nrw.social`, `norden.social`, `ruhr.social` — and this is exactly the setting an installation elsewhere changes: name your own, or set it **empty** (`TAG_SOURCE_SERVERS=""`) to offer none, which is the honest answer on an intranet that reaches none of them. A member may still type an address themselves; a server the operator blocks at `/admin/fediverse` is never offered whatever this says. `FETCH_EXTERNAL_TAG_POSTS=false` overrides it entirely — nothing is offered and nothing is asked |
