...
| `OPERATOR_HANDLE` | `wintermeyer` | The @handle of the person your media kit (`/system/media-kit`) names as the press contact, and the one an investor writes to on `/system/investors`. The media kit links their profile for the remaining contact details; the investor page states no address at all and offers a message on your own installation instead. Both appear only when the handle really belongs to a member of *your* installation, so leaving the default set costs you nothing but a missing link. `""` renders neither |
