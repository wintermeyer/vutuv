...
| `TAG_TRENDING_VET_LIMIT` | `8` | How many sample requests one pass may spend on candidates, loudest first. Candidates past it are not offered that pass |
| `TAG_TRENDING_CENSUS_PER_HOST` | `2` | How many of those sample requests may go to one server in a pass. Each candidate is sampled on whichever server that listed it has been asked least so far, so the load spreads rather than landing on the biggest one; a candidate whose servers have all had their share waits for the next pass. Before this cap one pass put seven of its eighteen requests on mastodon.social |
