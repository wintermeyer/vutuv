...
| `SOURCE_URL` | `https://github.com/wintermeyer/vutuv` | Where the source of the software you run can be read — the footer's "Source" link, the bug-report links on the error pages and in the developer docs, and the `source_url` / `repository` fields both API discovery documents publish. **Change this if you run a modified vutuv:** it is AGPL, so the source your users are pointed at has to be *yours*, and ours is no longer an honest answer once you have patched anything |
