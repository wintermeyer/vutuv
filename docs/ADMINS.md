...
| `MASTODON_API_ENABLED` | `true` | `false` turns the Mastodon-compatible phone-client adapter off on **both** hosts it is served from. It answers on `PHX_HOST` — which is the address members actually type into an app — and on `mastodon.<PHX_HOST>`, the canonical origin this app advertises. The subdomain is optional: it needs a DNS record, a TLS certificate and an nginx `server_name`, and without them the adapter still works on the main host. A redirect between the two would not do, because HTTP libraries drop the `Authorization` header across a host change. Handles and ActivityPub actors are on `PHX_HOST` either way. Access is **off per identity by default**: each member turns it on under `/settings/apps`, each organization owner under `/organizations/<slug>/apps`, and either can turn it off again, which takes effect on the next request. `/system/mastodon` is the member-facing help page |
| `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` | *(unset)* | The Web Push key pair for Mastodon-compatible phone clients, so a member's phone is woken by a notification instead of the app polling for one — which is the cheaper side for your server too. **Leave unset and push is simply off**: the endpoints answer 403 and nothing is sent, which is what an intranet installation with no reachable push service wants. Generate a pair once with `mix run -e 'IO.inspect Vutuv.MastodonApi.WebPush.generate_keys()'`, keep the private key secret, and set `VAPID_SUBJECT` to a `mailto:` address the push services can reach you at. A push carries **no content** — it names the notification's kind and id, never a word of what was written; the client fetches the rest over the authenticated API |
