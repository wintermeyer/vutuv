...
| `MASTODON_API_ENABLED` | `true` | `false` disables the Mastodon-compatible phone-client adapter on `mastodon.<PHX_HOST>`. It needs a DNS record, TLS certificate and nginx `server_name` for that subdomain. Handles and ActivityPub actors remain on `PHX_HOST`; users enter the `mastodon.` host in their app. Members and organization owners can disable client access again for each identity independently |
