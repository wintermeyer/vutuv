...
| `VIDEO_UPLOADERS` | `admins` | Who may attach a clip. `admins` while the feature is new: nobody else sees a picker, both APIs refuse their clips, and the Mastodon instance answer names no video types. `members` opens it to everyone. Playback is never gated: a clip an admin posted plays for all |
...
| `OPERATOR_HANDLE` | `wintermeyer` | The @handle of the person your media kit (`/system/media-kit`) names as the press contact, and the one an investor writes to on `/system/investors`. The media kit links their profile for the remaining contact details; the investor page names the operator's address (`OPERATOR_EMAIL`) and offers a message on your own installation beside it. Both appear only when the handle really belongs to a member of *your* installation, so leaving the default set costs you nothing but a missing link. `""` renders neither |
...
| `DATA_LOCATION` | `Deutschland` | Where your installation's data physically lives, the start page's answer to "Where does my data live?" ("on our own servers in X, in no foreign cloud"). **Set it to your own country or data centre, or to an empty value to drop that question entirely** — which is what you must do if you run vutuv on rented cloud infrastructure, since the sentence says *our own servers*. The cookie question beside it is a property of the software and stays whatever you set here: vutuv sets one first-party cookie and loads nothing from another host |
| `LANDING_EXAMPLE_PROFILE_URL` | `https://vutuv.de/wintermeyer` | The one profile the start page's questions point at: the example under "Can I look at profiles and posts without signing up?", and its JSON sibling as the example under "Is there an API?". A full URL, because the default has to keep working on an installation that has no filled-in profile yet — pointing at the reference installation is more useful there than a dead local link. Point it at one of your own members once you have one, or set it empty to drop both links (the questions, their answers and the developer-documentation link stay) |
| `WELCOME_SUGGESTIONS` | `de=@wintermeyer,@tagesschau@ard.social,…` (six accounts, see `config/config.exs`) | The accounts a brand-new member is offered to follow on the last step of the welcome window, so their feed has something in it before they go looking. One group per locale, groups separated by `;` and addresses by `,`: `de=@someone,@else@example.org;en=@third@example.org`. `@name` with no host means a member of **your** installation and drops off the list if nobody here is called that, which is why the German default costs you nothing; `@name@host` is an account elsewhere and is shown only to members who take part in the fediverse (and never while `FEDIVERSE_ENABLED=false`). A locale with no list of its own gets a two-step window instead of three, and `WELCOME_SUGGESTIONS=""` drops the step everywhere |
| `FEDIVERSE_ENABLED` | `true` | `false` turns follow-only ActivityPub federation off entirely (endpoints 404, nothing is delivered, and the sign-up form and the start page's technical section drop their Fediverse lines) — set it on intranet installations |
...
| `MEDIA_JOB_RETENTION_DAYS` | `90` | How long the **media-job log** at `/admin/media` keeps a row, in days. Every step the photo scan, the video conversion and the screenshot capture run writes one row saying what it worked on, how long it took and how it ended, so the table grows with every upload your installation handles. This is operational data, not personal data: the question is how far back you want to be able to look at a queue that misbehaved, not how long something may be kept. The daily sweeper deletes anything older |
| `FETCH_EXTERNAL_TAG_POSTS` | `true` | Whether a followed tag may bring posts from the **other servers a member named for it** (issue #2126). A hashtag has no inbox to subscribe to, so this is a pull: a background job reads each named server's public tag timeline over its Mastodon-compatible REST API — no account, no credentials — and keeps the newest handful as plain text plus a link to the original. **Never a picture**, so nothing here reaches the AI image gate, and never anything marked sensitive or sitting behind a content warning. The operator blocklist at `/admin/fediverse` is consulted at fetch time, so blocking a server stops the pull from it even for members who named it earlier. `false` asks nobody anything and does not start the job — set it on installations that must not call out (intranets); what was already fetched keeps rendering until its cap pushes it out |
| `EXTERNAL_TAG_CADENCE` | `5:10:180` | How often one (tag, server) pair is asked, as `target:min:max` — the number of new posts a fetch aims to find, then the floor and the ceiling in **minutes**. The pace finds itself: more than the target arrived and the wait halves, none arrived and it doubles, so a tag in the middle of a news event settles at ten minutes and a quiet local one drifts out to three hours with no list of "busy tags" for anybody to maintain. The floor is the neighbourly number — it decides how often a stranger's server hears from you — so raise it to be a quieter guest |
| `EXTERNAL_TAG_POST_CAPS` | `20:10000` | What bounds that cache, as `per_tag:total`: posts kept for one tag (its servers share those slots, newest wins) and rows in the whole table. Nothing in it was written here, so it is a cache with two ceilings rather than an archive — and the posts of a (tag, server) pair are deleted outright when the last member who wanted that pair drops it |
| `EXTERNAL_TAG_FETCH_BUDGET` | `20:5` | The ceilings on one such run (every two minutes), as `batch:per_host`: how many (tag, server) pairs may be asked at all, and how many of them may belong to a single server. The per-server half is the neighbourly one — twenty busy tags naming one popular server must not spend its rate limit in a single run — and what it holds back is what the next run serves first, so nothing starves. The log says how many pairs a run held back |
| `TAG_SOURCE_SERVERS` | ten servers | Which servers the **"Where should this tag come from?"** panel offers a member (issue #2128), comma-separated hostnames. The shipped ten are the vutuv.de list — `mastodon.social`, `mastodon.world`, `mastodon.online`, `hachyderm.io`, `troet.cafe`, `social.tchncs.de`, `det.social`, `nrw.social`, `norden.social`, `ruhr.social` — and this is exactly the setting an installation elsewhere changes: name your own, or set it **empty** (`TAG_SOURCE_SERVERS=""`) to offer none, which is the honest answer on an intranet that reaches none of them. **Listing a server here means trusting it to relay posts from other servers**: an honest Mastodon server checks the signatures of what it passes on, and yours are the only servers whose word about somebody else's member vutuv takes (issue #2174). A member may still type an address themselves, but such a server only brings posts by its own members; a server the operator blocks at `/admin/fediverse` is never offered whatever this says. `FETCH_EXTERNAL_TAG_POSTS=false` overrides it entirely — nothing is offered and nothing is asked |
| `TAG_SOURCES_PER_FOLLOW` | `3` | How many **other** servers one followed tag may name; this installation is always among its sources and never counts against it. A **per-member fairness limit**, not a ceiling on your outbound rate — that is `EXTERNAL_TAG_FETCH_BUDGET`, and pairs are deduplicated across members, so ten members naming three different servers for one tag is still thirty pairs. What it bounds is one member's share: every (tag, server) pair is asked forever, up to 144 times a day at the `EXTERNAL_TAG_CADENCE` floor. Measured over three tags, two servers already carried the whole fresh end and further ones returned older material — raise it if your members want reach more than your server wants quiet |
| `FETCH_TRENDING_TAGS` | `true` | Whether the feed's tag card offers **tags that are suddenly busy on the servers in `TAG_SOURCE_SERVERS`** (issue #2129), one press to follow. Every Mastodon server publishes its trending tags with a seven-day history, without a login, so a pass reads that list from each of your servers and offers what today's count puts far above the six days before it — busy is not enough, or `#xbox` at 130 uses a day would stand there forever. Two defences on top, because the loudest tag is often a machine: a candidate is only offered if several servers list it **and** a sample of its own timeline shows a real crowd rather than one domain's bot accounts, which the remote server flags itself. `false` asks nobody anything and draws no row; the last answers age out on their own. Off automatically when `FETCH_EXTERNAL_TAG_POSTS=false` or `TAG_SOURCE_SERVERS=""` — a server you may not fetch from is not one to take a recommendation from either |
| `TAG_TRENDING_INTERVAL_MINUTES` | `30` | How long between two such passes. One small `GET` per server per pass, plus at most eight more to sample the candidates it is about to offer (`TAG_TRENDING_VET_LIMIT`) — so the shipped ten servers cost about 860 requests a day in total, against 144 a day for a *single* (tag, server) pair at the `EXTERNAL_TAG_CADENCE` floor. Every server is asked in the same pass on purpose: "how many servers is this tag busy on" is the spread defence, and a pass that asked three of ten would answer it from three. An offer older than four intervals is dropped rather than shown, so a stopped job leaves no stale news on the card |
| `TAG_TRENDING_OFFERS` | `5` | How many of those tags the card draws. Tags the reader already follows are left out of their own row, so a member who follows most of what is busy sees a shorter list or none |
| `TAG_TRENDING_MIN_USES` | `25` | The first of the thresholds that decide what counts as suddenly busy: today's uses, summed over the servers listing the tag, must reach this. The shipped values were calibrated against the shipped ten servers; a smaller neighbourhood sees smaller numbers and may need a lower floor. All of these take a positive whole number, and an unreadable value keeps the default and says so on the console |
| `TAG_TRENDING_SPIKE_FACTOR` | `5` | How many times the median of the six days before today's count must be. This is what keeps a tag that is busy every day (`#xbox` at 130 uses against a median of 78) off the row |
| `TAG_TRENDING_MIN_SERVERS` | `2` | How many of your servers must list a tag as trending at all. **An installation that reads one server must set this to `1`**, or nothing can ever qualify and the row stays empty without a word |
| `TAG_TRENDING_MIN_AUTHOR_HOSTS` | `4` | The bot defence: how many distinct author servers the sample of a candidate's timeline must show. A bot wave measured 1 or 2, ordinary tags 13 to 26 |
| `TAG_TRENDING_MAX_BOT_PERCENT` | `50` | The other half of it: the highest share of that sample that may come from accounts their own server flags as bots. A bot wave measured 98 %, ordinary tags at most 25 % |
| `TAG_TRENDING_MIN_SAMPLE` | `10` | Below this many sampled posts the sample says nothing and the tag is not offered |
| `TAG_TRENDING_VET_LIMIT` | `8` | How many sample requests one pass may spend on candidates: those never sampled first, then those sampled longest ago. A candidate the pass does not reach is offered on its last sample while that is younger than four intervals, so a small budget takes a few passes to get round everything rather than leaving the quieter tags out for good |
| `TAG_TRENDING_CENSUS_PER_HOST` | `2` | How many of those sample requests may go to one server in a pass. Each candidate is sampled on whichever server that listed it has been asked least so far, so the load spreads rather than landing on the biggest one; a candidate whose servers have all had their share waits for a later pass. Before this cap one pass put seven of its eighteen requests on mastodon.social |
| `TAG_SERVER_INFO_MAX_AGE_HOURS` | `24` | How long what this installation knows about another server (its account and post counts, its description, whether its public tag timeline can be read at all) stays fresh before the panel asks again. Nothing on that record moves fast, and the panel is the only thing that ever asks — so on an installation where nobody opens it, this feature makes **no** outbound requests of its own at all. Up to four requests per server per refresh: the tag timeline, which alone decides whether the server can be picked, then NodeInfo discovery, the NodeInfo document and Mastodon's instance document for the figures and the language badge, none of which a server has to serve to be usable |
...
| `MEDIA_KIT_MAX_MB` | `30` | The largest press photo or logo file a member or a page may offer for download. This is the one place on vutuv where the **original** is what leaves — a journalist downloads a file a printer can use, not a screen-sized copy — so the budget is far above the 4 MB an organization logo gets, and 30 MB is a full-frame camera JPEG with room to spare. The website uploads over the LiveView socket, so nginx's `client_max_body_size` only has to allow this much if you route uploads over plain HTTP as well. A press photo may only be a JPEG, PNG or WebP (the containers whose metadata we can strip with certainty, so a download never carries a GPS fix), and a logo an SVG or PNG |
| `MEDIA_KIT_MAX_PHOTOS` | `10` | Most press photos one member or one page may offer. Ten covers a portrait, a working shot and a handful of situations while still being a set somebody can look through |
...
- Set `WEB_PUSH_ENABLED=false`: Web Push needs no configuration and is
  ...
  keeps working.
- Set `WELCOME_SUGGESTIONS` to your own accounts, or to `""`: the shipped list
  names two accounts on other servers, and with `FEDIVERSE_ENABLED=false` those
  are never shown anyway — but the German default also suggests a member called
  `wintermeyer`, which on your installation is either somebody else or nobody.
  A list of your own (`de=@welcome-team`) is the useful setting here: it puts
  something in a new member's feed on their first day, which on an intranet is
  the whole feed.
...
- Set `FETCH_EXTERNAL_TAG_POSTS=false`: a member following a tag can name other
  servers it should also come from, and the standing job then reads those
  servers' public tag timelines. Off, the job does not even start and nothing is
  asked; members keep following tags, they simply get what your own installation
  wrote about them. `FEDIVERSE_ENABLED=false` does **not** cover this — it is a
  separate, deliberately plain HTTPS pull, not ActivityPub. The panel that picks
  those servers then offers none and says why, rather than listing ten servers
  nobody here can reach.
- Set `TAG_SOURCE_SERVERS` to your own servers if the pull stays on but the
  shipped ten are unreachable — an intranet with a Mastodon of its own is the
  case this exists for. `TAG_SOURCE_SERVERS=""` offers none while leaving a
  member free to type an address that does work from here.
- The "suddenly busy on other servers" row asks exactly those servers, so both
  of the settings above already switch it off; `FETCH_TRENDING_TAGS=false` turns
  it off on its own if you keep the pull but do not want the discovery row. If
  you keep it with a single internal server, set `TAG_TRENDING_MIN_SERVERS=1`:
  the shipped threshold wants a tag busy on at least two servers, so otherwise
  the row silently offers nothing.
...
"Where does my data live?" answer on your start page. Run on rented cloud
