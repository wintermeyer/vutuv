...
| `FEDIVERSE_POST_RETENTION_DAYS` | `183` | How long a post by an account one of your members follows may be cached here at the very most, in days. Since issue #1161 those posts appear in the follower's home feed, which means storing them; this is the ceiling the copy is deleted at whatever else happens, so it is the promise your privacy page can make. An upstream edit or deletion is honoured at once, and the cache is dropped entirely the moment nobody here follows the author any more — this is only the backstop |
