...
| `TAG_TRENDING_INTERVAL_MINUTES` | `30` | How long between two such passes. One small `GET` per server per pass, plus at most eight more to sample the candidates it is about to offer — so the shipped ten servers cost about 860 requests a day in total, against 144 a day for a *single* (tag, server) pair at the `EXTERNAL_TAG_CADENCE` floor. Every server is asked in the same pass on purpose: "how many servers is this tag busy on" is the spread defence, and a pass that asked three of ten would answer it from three. An offer older than four intervals is dropped rather than shown, so a stopped job leaves no stale news on the card |
...
- The "suddenly busy on other servers" row asks exactly those servers, so both
  ...
  it off on its own if you keep the pull but do not want the discovery row.
