...
| `INVESTOR_MINIMUM` | `300000` | The smallest investment `/system/investors` says is worth a conversation. Stated early on that page so an enquiry that ends at this number ends there rather than a week later. `0` drops the whole block, which is what an installation raising nothing wants |
| `INVESTOR_CURRENCY` | `EUR` | The currency `INVESTOR_MINIMUM` is stated in. `EUR`, `USD` and `GBP` render as symbols, anything else keeps its ISO code. The digits are grouped the way the reader's own language groups them (`300.000` / `300,000`); the symbol follows the amount in every language, as it does in the pay line on a job posting |
