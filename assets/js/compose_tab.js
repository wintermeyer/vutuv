...
//
// It is a link to `/feed#compose`, the composer deep link the profile card and
// the launcher shortcut already use, so from any other page it is a plain
// navigation and the arrival handler in keyboard_shortcuts.js opens the
// composer. On the feed itself a navigation would reload the page under the
// reader, so the press is taken here and the composer opened in place —
// through the same `focusComposer` the "n" shortcut uses, which clicks the
// feed's (hidden, on a phone) compose button and focuses the editor.
//
