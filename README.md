...
- **Daily activity report** (`Vutuv.Reports`): a basic operator metric for one German calendar day (Europe/Berlin via `Vutuv.BerlinTime.day_bounds_utc/1`) — confirmed-by-PIN new registrations (`email_confirmed?` accounts created that day) plus the day's posts, reposts, likes and bookmarks. Admins read it at `/admin/reports`, a time machine: `?date=YYYY-MM-DD` (defaulting to yesterday) with prev/next links and a date picker. `Vutuv.Reports.DailyReporter`, a supervised cron-style GenServer, schedules itself for the next **00:05 Berlin** and mails the previous day's report to the operator through the `Emailer` chokepoint (`daily_report_email/1`), **skipping any day whose every metric is zero**. Behind `config :vutuv, :daily_report_email` (off in tests, on by default).
...
- **Ordered links**: members arrange their profile links in the order they want instead of by creation date (`urls.position`, backfilled in creation order). The management page `/:slug/links` carries an owner-only ordering tool — drag a link by its handle, or use per-row up/down arrows that submit a real form so they also work without JavaScript and from the keyboard (`reorder` takes the full order via fetch, `move` nudges one link; both renumber positions 1..n server-side). The chosen order drives the profile preview, the links page and every agent-format sibling; new links append to the end.
