...
import { ROOT, BASE, CSS, HELPERS, loadContent, newContext, open, dress, record, pointer } from "./lib.mjs";
...
const HIDE_OTHER_STAMMTISCH = ({ post, needles }) => document.querySelectorAll("#feed-posts > div").forEach((d) => {
  if (!d.querySelector(`[id*="${post}"]`) && needles.some((n) => d.textContent.includes(n))) d.style.display = "none";
});
const needles = ["Elixir-Stammtisch", "Elixir meetup", c.post.line1.slice(0, 30)];
...
if (wanted("phones")) {
  ...
  for (const [name, url, anchor, state, hideMine] of [
    ...
    await dress(p);
    await p.evaluate(({ anchor, hideMine, post, needles }) => {
      window.tzHideCounts();
    ...
    }, { anchor, hideMine, post: POST, needles });
...
// hide the one real posting of the database copy and the tag chips only it brings
const HIDE_REAL_JOBS = (orgs) => {
...
const ORGS = [...new Set(c.jobs.map((j) => j.org))];
