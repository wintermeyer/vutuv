defmodule Vutuv.Translations.PrecomputeTest do
  ...
  # Every locale this installation serves except the one a post is written in.
  # Derived rather than spelled out, so adding a third interface language moves
  # these expectations with it instead of turning the file red.
  defp targets(language) do
    {:ok, config} = Application.fetch_env(:vutuv, VutuvWeb.Endpoint)
    config[:locales] |> List.delete(language) |> Enum.sort()
  end

  defp target_languages(post) do
    post |> jobs_for() |> Enum.map(& &1.target_language) |> Enum.sort()
  end

  defp translate_into_every_target(post, language) do
    for target <- targets(language) do
      {:ok, _} = Translations.store_translation(post, target, %{body: "Morning.", model: "m"})
    end
  end
  ...
  describe "picking candidates" do
    test "opens a background job for the locale the post is not written in" do
      ...
      assert Translations.enqueue_background() == length(targets("de"))
      assert target_languages(post) == targets("de")

      for job <- jobs_for(post) do
        assert %TranslationJob{priority: @background, status: "pending"} = job
      end
    end
    ...
    test "never translates a post into its own language" do
      ...
      assert Repo.all(from(j in TranslationJob, select: j.target_language)) |> Enum.sort() ==
               targets("de")
    end
    ...
    test "covers BOTH directions — every locale this installation serves" do
      ...
      assert Translations.enqueue_background() ==
               length(targets("de")) + length(targets("en"))

      assert target_languages(german) == targets("de")
      ...
      assert target_languages(english) == targets("en")
    end
    ...
    test "a post that already has a fresh translation opens no job" do
      post = german_post()
      translate_into_every_target(post, "de")
  ...
  describe "the sweep's clock" do
    test "a considered post leaves the work list — including the do-nothing outcome" do
      ...
      assert Translations.enqueue_background(limit: 1) == length(targets("de"))
      assert target_languages(other) == targets("de")
    end
    ...
    test "a stamped post is not reconsidered until the interval is up" do
      ...
      assert Translations.enqueue_background() == 0
      assert length(jobs_for(post)) == length(targets("de"))
    end
    ...
    test "a failed translation is retried once the reconsider interval has passed" do
      ...
      assert Translations.enqueue_background() == length(targets("de"))
      assert length(jobs_for(post)) == 2 * length(targets("de"))
    end
  end
  ...
  describe "the backlog cap" do
    test "a full backlog stops the round dead" do
      ...
      opened = 3 * length(targets("de"))

      assert Translations.enqueue_background(backlog_cap: 2) == opened
      assert Translations.background_backlog() == opened
