defmodule Vutuv.TagsTest do
  ...
  describe "member_reach_by_name/1" do
    defp named_tag(base) do
      name = unique_tag_name(base)
      insert(:tag, name: name, slug: Vutuv.SlugHelpers.tagify(name))
    end

    test "counts a member once however many of the topics they carry" do
      elixir = named_tag("Elixir")
      linux = named_tag("Linux")

      both = insert(:activated_user)
      insert(:user_tag, user: both, tag: elixir)
      insert(:user_tag, user: both, tag: linux)

      one = insert(:activated_user)
      insert(:user_tag, user: one, tag: linux)

      # Summing the per-topic counts would say three. The point of the number
      # the sign-up form shows is how many people it reaches, not how many
      # rows match, so the one member holding both topics counts once.
      assert Tags.member_reach_by_name([elixir.name, linux.name]) == 2
    end

    test "leaves out an account a tag page would not list" do
      tag = named_tag("Rust")
      insert(:user_tag, user: insert(:activated_user), tag: tag)
      insert(:user_tag, user: insert(:user), tag: tag)

      assert Tags.member_reach_by_name([tag.name]) == 1
    end

    test "resolves a spelling variant to the topic it names" do
      tag = named_tag("Elixir")
      insert(:user_tag, user: insert(:activated_user), tag: tag)

      assert Tags.member_reach_by_name([String.downcase(tag.name)]) == 1
    end

    test "a name nothing matches yet adds nobody" do
      tag = named_tag("Elixir")
      insert(:user_tag, user: insert(:activated_user), tag: tag)

      assert Tags.member_reach_by_name([
               tag.name,
               "Origami #{System.unique_integer([:positive])}"
             ]) ==
               1
    end

    test "nothing typed reaches nobody" do
      assert Tags.member_reach_by_name([]) == 0
    end
  end
end
