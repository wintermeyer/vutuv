defmodule VutuvWeb.ApiV2.ImagesApiTest do
  ...
  test "upload, attach to a post, and the audience proxy applies", %{
    ...
    conn1 =
      conn
      |> authed(token)
      |> post("/api/2.0/me/post_images", %{"image" => jpeg!(tmp), "alt" => "Greenfield"})
    ...
    conn2 =
      ...
      |> post("/api/2.0/posts", Jason.encode!(%{body: "With image", image_ids: [image_id]}))
  ...
  test "deleting a pending image; attached ones refuse", %{
    ...
    conn1 = conn |> authed(token) |> post("/api/2.0/me/post_images", %{"image" => jpeg!(tmp)})
    %{"id" => image_id} = json_response(conn1, 201)

    conn2 = build_conn() |> authed(token) |> delete("/api/2.0/me/post_images/#{image_id}")
    ...
    conn3 = build_conn() |> authed(token) |> delete("/api/2.0/me/post_images/#{attached.id}")
  ...
  test "posts:read cannot upload", %{conn: conn, user: user, tmp: tmp} do
    ...
    conn = conn |> authed(read_token) |> post("/api/2.0/me/post_images", %{"image" => jpeg!(tmp)})
