defmodule VutuvWeb.ApiV1.PostsApiTest do
  ...
  describe "create / read / update / delete" do
    test "the full lifecycle of a post", %{conn: conn, me: me, token: token} do
      conn1 =
        json_req(conn, :post, token, "/api/v1/posts", %{
          body: "Hello **API**",
          tags: "elixir, phoenix"
        })
      ...
      conn2 = get(authed(build_conn(), token), "/api/v1/posts/#{id}")
      assert json_response(conn2, 200)["id"] == id

      conn3 = json_req(build_conn(), :patch, token, "/api/v1/posts/#{id}", %{body: "Edited"})
      assert json_response(conn3, 200)["body_markdown"] == "Edited"

      conn4 = delete(authed(build_conn(), token), "/api/v1/posts/#{id}")
      assert conn4.status == 204
      assert Posts.get_post(me, id) == nil
    end

    test "an empty post is a 422", %{conn: conn, token: token} do
      conn = json_req(conn, :post, token, "/api/v1/posts", %{body: ""})
    ...
    test "audience denials apply: a connections-only post hides from strangers", %{
      ...
      conn2 = get(authed(build_conn(), stranger_token), "/api/v1/posts/#{id}")
      assert conn2.status == 404

      connect!(me, other)
      conn3 = get(authed(build_conn(), stranger_token), "/api/v1/posts/#{id}")
    ...
    test "cannot edit someone else's post", %{conn: conn, other: other, token: token} do
      ...
      conn = json_req(conn, :patch, token, "/api/v1/posts/#{post.id}", %{body: "hijack"})
  ...
  describe "archive and feed" do
    test "the author archive lists posts with ids", %{conn: conn, other: other, token: token} do
      ...
      conn = get(authed(conn, token), "/api/v1/users/#{other.active_slug}/posts")
    ...
    test "the feed shows followed authors and pages by cursor", %{
      ...
      conn1 = get(authed(conn, token), "/api/v1/feed?limit=2")
      ...
      conn2 =
        get(
          ...
          "/api/v1/feed?limit=2&cursor=#{URI.encode_www_form(page1["next_cursor"])}"
        )
    ...
    test "a tampered cursor is a 400", %{conn: conn, token: token} do
      conn = get(authed(conn, token), "/api/v1/feed?cursor=garbage")
  ...
  describe "replies" do
    test "replying to a public post", %{conn: conn, other: other, token: token} do
      ...
      conn1 =
        json_req(conn, :post, token, "/api/v1/posts/#{parent.id}/replies", %{body: "Nice one"})

      reply = json_response(conn1, 201)
      assert reply["in_reply_to"]["url"] =~ parent.id

      conn2 = get(authed(build_conn(), token), "/api/v1/posts/#{parent.id}")
    ...
    test "a restricted parent refuses replies with a 409", %{conn: conn, me: me, token: token} do
      ...
      conn = json_req(conn, :post, token, "/api/v1/posts/#{parent.id}/replies", %{body: "reply"})
  ...
  describe "engagement" do
    test "like, engagement doc, unlike", %{conn: conn, other: other, token: token} do
      ...
      conn1 = put(authed(conn, token), "/api/v1/posts/#{post.id}/like")
      body = json_response(conn1, 200)
      assert body["likes"] == 1
      assert body["liked?"] == true

      conn2 = put(authed(build_conn(), token), "/api/v1/posts/#{post.id}/like")
      assert json_response(conn2, 200)["likes"] == 1

      conn3 = delete(authed(build_conn(), token), "/api/v1/posts/#{post.id}/like")
    ...
    test "reposting a restricted post is a 409", %{conn: conn, me: me, token: token} do
      ...
      conn = put(authed(conn, token), "/api/v1/posts/#{post.id}/repost")
      assert conn.status == 409
    end
    ...
    test "audience locking: a reposted post refuses restriction", %{
      ...
      conn =
        json_req(conn, :patch, token, "/api/v1/posts/#{post.id}", %{
          body: post.body,
          denials: [%{"wildcard" => "non_connections"}]
        })
    ...
    test "posts:read suffices for reading but not engaging", %{
      ...
      assert get(authed(conn, read_token), "/api/v1/posts/#{post.id}").status == 200
      assert put(authed(build_conn(), read_token), "/api/v1/posts/#{post.id}/like").status == 403
    end
  end
end
