defmodule VutuvWeb.ApiV1.ProfileApiTest do
  ...
  describe "PATCH /api/v1/me" do
    test "updates the whitelisted profile fields", %{conn: conn, write_token: token} do
      conn = json_patch(conn, token, "/api/v1/me", %{headline: "New **headline**"})
    ...
    test "ignores fields outside the API contract", %{conn: conn, user: user, write_token: token} do
      conn =
        json_patch(conn, token, "/api/v1/me", %{
          first_name: "Renamed",
    ...
    test "invalid values are a 422 with field errors", %{conn: conn, write_token: token} do
      conn = json_patch(conn, token, "/api/v1/me", %{birthdate: "not-a-date"})
  ...
  describe "GET /api/v1/users/:slug/<section>" do
    ...
    test "the email list reads through the viewer's eyes", %{
      ...
      conn1 = get(authed(conn, token), "/api/v1/users/#{other.active_slug}/emails")
      assert json_response(conn1, 200)["entries"] == ["public@example.com"]

      conn2 = get(authed(build_conn(), token), "/api/v1/users/#{user.active_slug}/emails")
      assert "mine-private@example.com" in json_response(conn2, 200)["entries"]
    end

    test "unknown slugs 404", %{conn: conn, read_token: token} do
      conn = get(authed(conn, token), "/api/v1/users/nobody/links")
      assert conn.status == 404
    end
  end

  describe "section CRUD on /api/v1/me" do
    test "create, update, delete a work experience", %{conn: conn, write_token: token} do
      conn1 =
        json_post(conn, token, "/api/v1/me/work_experiences", %{
          title: "Developer",
          organization: "ACME",
          start_year: 2024,
          start_month: 3
      ...
      conn2 =
        json_patch(build_conn(), token, "/api/v1/me/work_experiences/#{id}", %{
          title: "Senior Developer"
        })

      assert json_response(conn2, 200)["entry"]["title"] == "Senior Developer"

      conn3 = delete(authed(build_conn(), token), "/api/v1/me/work_experiences/#{id}")
    ...
    test "validation errors are a 422 with field errors", %{conn: conn, write_token: token} do
      conn = json_post(conn, token, "/api/v1/me/work_experiences", %{title: "No org"})
    ...
    test "creates a link and a phone number", %{conn: conn, user: user, write_token: token} do
      conn1 =
        json_post(conn, token, "/api/v1/me/links", %{
      ...
      conn2 =
        json_post(build_conn(), token, "/api/v1/me/phone_numbers", %{
    ...
    test "cannot touch someone else's entries", %{conn: conn, write_token: token} do
      ...
      conn1 = json_patch(conn, token, "/api/v1/me/work_experiences/#{work.id}", %{title: "x"})
      assert conn1.status == 404

      conn2 = delete(authed(build_conn(), token), "/api/v1/me/work_experiences/#{work.id}")
  ...
  describe "tags" do
    test "add, list and remove a tag", %{conn: conn, user: user, write_token: token} do
      conn1 = json_post(conn, token, "/api/v1/me/tags", %{name: "Phoenix"})
      assert %{"entry" => %{"id" => id, "name" => "Phoenix"}} = json_response(conn1, 201)

      conn2 = get(authed(build_conn(), token), "/api/v1/users/#{user.active_slug}/tags")
      assert [%{"name" => "Phoenix"}] = json_response(conn2, 200)["entries"]

      conn3 = delete(authed(build_conn(), token), "/api/v1/me/tags/#{id}")
      assert conn3.status == 204

      assert json_response(
               get(authed(build_conn(), token), "/api/v1/users/#{user.active_slug}/tags"),
               200
             )["entries"] == []
    end

    test "a missing name is a 400", %{conn: conn, write_token: token} do
      conn = json_post(conn, token, "/api/v1/me/tags", %{})
