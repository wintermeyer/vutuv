defmodule VutuvWeb.AgentFormatTest do
  ...
  describe "the language hint" do
    ...
    test "no hint for a language we have no translation for, or without the header" do
      # `zz` here, not `fr`: this hint keys off `Gettext.known_locales/1` rather
      ...
      # to be a language with no catalog at all, and `zz` is unassignable in
      # ISO 639-1, so it can never become the next language somebody adds.
      conn =
        build_conn()
        |> put_req_header("accept-language", "zz-ZZ,zz;q=0.9")
        |> get("/agent_tester.md")
