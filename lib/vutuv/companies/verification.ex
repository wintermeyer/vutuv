defmodule Vutuv.Companies.Verification do
  ...
  proving control of the DOMAIN itself (never merely an address on it):
    ...
    * `dns` — a `vutuv-company-verify=<token>` TXT record.
  ...
  owns the **company verification scheme** (the `vutuv-company-verify=` DNS
  prefix and the `/.well-known/vutuv-company-verify.txt` file — deliberately
  distinct from the `vutuv-verify=` scheme personal-webpage links use, so a
  proof for a link never doubles as a proof for a company on the same host),
  the company outbound-call gate (`:verify_company_domains`) and the company
  test seams (`:companies_dns_resolver` / `:companies_req_options`). Off =
  company domain verification is disabled on this installation (no outbound
  calls).
  """

  alias Vutuv.WebVerification

  @dns_prefix "vutuv-company-verify="
  @well_known_path "/.well-known/vutuv-company-verify.txt"
  ...
  @doc "Whether `host` publishes a `vutuv-company-verify=<token>` TXT record."
  ...
  @doc """
  Whether `https://host/.well-known/vutuv-company-verify.txt` serves exactly `token`.
  """
