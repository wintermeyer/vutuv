defmodule Vutuv.PostAnalytics do
  ...
  def for_post(%Post{} = post, opts \\ []) do
    ...
    %{range: range, unit: unit, buckets: points, totals: totals, peak: peak}
  end
  ...
  defp events(post_id, unit, first_at, now) do
    ...
    {:ok, binary_id} = Ecto.UUID.dump(post_id)

    %{rows: rows} =
      Repo.query!(sql, [binary_id, unit, DateTime.to_naive(first_at), DateTime.to_naive(now)])
