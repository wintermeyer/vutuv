defmodule VutuvWeb.Plug.ApiV1Auth do
  @moduledoc """
  Bearer-token authentication for `/api/v1` (see `Vutuv.ApiAuth`).
  ...
  defp rate_check(token) do
    {limit, window_ms} = Application.get_env(:vutuv, :api_v1_rate_limit, @default_rate_limit)
  ...
  defp rate_limited(conn) do
    {_limit, window_ms} = Application.get_env(:vutuv, :api_v1_rate_limit, @default_rate_limit)
