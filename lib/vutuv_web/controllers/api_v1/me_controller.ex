defmodule VutuvWeb.ApiV1.MeController do
  ...
  `PATCH /api/v1/me` — update the plain profile fields. The username
  ...
  def show(conn, _params) do
    ...
    ApiV1.send_json(conn, ProfileDoc.build(user, viewer: user))
  end
  ...
  def update(conn, params) do
    ...
    case Accounts.update_user(user, Map.take(params, @updatable_fields)) do
      {:ok, user} -> ApiV1.send_json(conn, ProfileDoc.build(user, viewer: user))
