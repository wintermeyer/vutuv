defmodule VutuvWeb.ApiV1.NotificationController do
  @moduledoc """
  The member's notification feed (`GET /api/v1/notifications`, cursor-
  paginated, with the unread count) and the read marker (`POST
  /api/v1/notifications/read`). The feed is `Vutuv.Activity`'s derived
  ...
  def index(conn, params) do
    ...
    case ApiV1.decode_cursor(params["cursor"]) do
      {:ok, cursor} ->
        page = Activity.notifications_page(me.id, cursor: cursor, limit: ApiV1.page_limit(params))
