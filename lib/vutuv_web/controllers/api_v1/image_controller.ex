defmodule VutuvWeb.ApiV1.ImageController do
  @moduledoc """
  Post image upload over the API: `POST /api/v1/me/post_images`
  ...
  `DELETE /api/v1/me/post_images/:id`. Same store, sweeper and audience
  ...
  def create(conn, %{"image" => %Plug.Upload{} = upload} = params) do
    ...
    case Posts.create_pending_image(user, upload) do
      {:ok, image} ->
        ApiV1.send_json(conn, image_doc(set_alt(image, params["alt"])), 201)
