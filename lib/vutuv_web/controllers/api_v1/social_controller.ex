defmodule VutuvWeb.ApiV1.SocialController do
  @moduledoc """
  ...
  defp follow_list(conn, slug, side, params) do
    case ApiV1.fetch_visible_user(slug, conn.assigns.current_user) do
      {:ok, user} ->
  ...
  def follow(conn, %{"slug" => slug}) do
    ...
    with {:ok, user} <- ApiV1.fetch_visible_user(slug, viewer) do
      cond do
        user.id == viewer.id ->
        ...
        Social.user_follows_user?(viewer.id, user.id) ->
          ...
          ApiV1.send_json(conn, follow_doc(user, viewer))
  ...
  defp create_follow(conn, viewer, user) do
    case Social.follow(viewer, user.id) do
      {:ok, _follow} ->
        ApiV1.send_json(conn, follow_doc(user, viewer), 201)
  ...
  def unfollow(conn, %{"slug" => slug}) do
    ...
    with {:ok, user} <- ApiV1.fetch_visible_user(slug, viewer),
    ...
    with {:ok, user} <- ApiV1.fetch_visible_user(slug, viewer) do
      case Social.request_connection(viewer, user) do
        {:ok, %Connection{status: "accepted"} = connection} ->
          ...
          ApiV1.send_json(conn, connection_doc(connection, viewer))

        {:ok, %Connection{} = connection} ->
          ApiV1.send_json(conn, connection_doc(connection, viewer), 201)
