defmodule VutuvWeb.CompanyImageController do
  @moduledoc """
  The authorizing company-image proxy (issue #929): every logo / description
  image byte is served through here, so a page's visibility guards its images
  too (a pending or frozen company's logo is owner/admin-only, and flips public
  ...
  def show(conn, %{"token" => token, "version" => version_file}) do
    with version when not is_nil(version) <- parse_version(version_file),
         image when not is_nil(image) <- Companies.get_image_by_token(token),
         true <- Companies.image_visible_to?(image, conn.assigns[:current_user]) do
  ...
  # Reuses the post-image serving mode (:post_image_serving), so a company image
  ...
  defp serve(conn, token, version) do
    ...
    case Application.get_env(:vutuv, :post_image_serving, :send_file) do
      :accel_redirect ->
        accel_path = CompanyImageStore.accel_path(token, version)
      ...
      _send_file ->
        case CompanyImageStore.version_path(token, version) do
          nil ->
            not_found(conn)
