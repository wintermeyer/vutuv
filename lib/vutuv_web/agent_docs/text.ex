defmodule VutuvWeb.AgentDocs.Text do
  ...
  def render(%{type: "organization"} = doc) do
    [
      ...
      [
        ...
        # `fediverse_fact/1` writes the member facts, which carry no dash here,
        # so this list adds its own.
        doc[:fediverse] && "- #{fediverse_fact(doc[:fediverse])}"
      ]
  ...
  def render(%{type: "media_kit"} = doc) do
    [
      ...
      doc.link_intro,
      Enum.map(doc.link_badges, &"- #{&1.name}: #{&1.url}"),
      Enum.map(doc.link_snippets, &"#{&1.name} (#{&1.language})\n\n#{&1.code}"),
      "Colours",
