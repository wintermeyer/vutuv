defmodule VutuvWeb.WorkExperienceHTML do
  ...
  # Issue #916: the volunteer category also welcomes hobbies and general
  # volunteer work (in IT especially, those are often not recognized as an
  # Ehrenamt), so the label names all three.
  def kind_name("volunteer"), do: gettext("Volunteering & hobbies")
  ...
  def kind_label("volunteer"), do: gettext("Volunteering & hobbies")
  ...
  claim an undated entry never made.

  Any `Vutuv.Profiles` entry with the four date columns: a work experience, an
  education entry. One definition, because this rule was spelled out four times
  with three different guards, and the loosest of them (`start_year` and
  `end_year` both nil) still called a months-only entry "Present".
  """
