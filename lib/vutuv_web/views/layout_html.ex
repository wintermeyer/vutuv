defmodule VutuvWeb.LayoutHTML do
  ...
  def toast(assigns) do
    ...
    <div class={@class} role={@role}>
      ...
      <button
        type="button"
        class="ml-auto shrink-0 text-slate-600 dark:text-slate-400"
        data-toast-close
  ...
  def ad_banner_box(assigns) do
    ...
    <div class="flex items-start gap-3 rounded-xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
      ...
      <%= case @banner do %>
        ...
        <% :house -> %>
          <p class="mb-0 min-w-0 text-sm text-slate-700 dark:text-slate-300">
            ...
            <.link href={~p"/ads"} class="font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300">
      ...
      <button
        ...
        class="ml-auto shrink-0 text-slate-600 dark:text-slate-400 dark:hover:text-slate-300"
      >
        ✕
      </button>
    </div>
