defmodule VutuvWeb.UserHTML do
  ...
  **The link appears only when the file is really on disk** (`Avatar.large_url/1`
  ...
  # Friendica has no glyph in the icon sets we draw from, so it gets its logo's
  # shape: a rounded square with an F cut out of it.
  defp social_icon_path("Friendica"),
    do:
      "M6 2h12a4 4 0 0 1 4 4v12a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V6a4 4 0 0 1 4-4ZM8 6v12h3v-4.5h4V11h-4V9h6V6Z"
  ...
  defp social_icon_path("Pixelfed"),
    do:
  ...
  # BookWyrm has no glyph of its own in the icon sets we draw from, so it gets
  # an open book (Heroicons solid book-open, MIT).
  defp social_icon_path("BookWyrm"),
    do:
  ...
  # The handle as shown to a visitor. The schema owns the leading "@" — this was
  # a hand-copied provider list until Pixelfed was added to the schema's table
  # and not to this one, which rendered the same account two ways.
