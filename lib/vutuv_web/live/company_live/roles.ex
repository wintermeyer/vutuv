defmodule VutuvWeb.CompanyLive.Roles do
  @moduledoc """
  The owner-only company team roster (`/companies/:slug/roles`, issue #930). Add
  a member by `@handle`/email (with a live typeahead), pick their role
  (owner / admin / recruiter), change a role, or remove someone. The company
  always keeps ≥ 1 owner, so removing or demoting the last owner is refused.
  Embedded via `live_render` from `VutuvWeb.CompanyController`, which gates it on
  an owner.
  """

  use VutuvWeb, :live_view

  import VutuvWeb.CompanyComponents, only: [manage_header: 1, role_label: 1]
  ...
  def mount(_params, session, socket) do
    ...
    {:ok,
     ...
     |> assign(:page_title, gettext("Team – %{name}", name: company.name))
     |> assign(:identifier, "")
  ...
  defp load_roles(socket) do
    assign(socket, :roles, Companies.list_roles(socket.assigns.company))
  end
  ...
  def handle_event("suggest", %{"identifier" => term} = params, socket) do
    ...
    {:noreply,
     ...
     |> assign(:suggestions, Companies.suggest_members(term, exclude))}
  end
  ...
  def handle_event("add_member", %{"identifier" => identifier} = params, socket) do
    ...
    case identifier |> String.trim() |> Accounts.get_user_by_handle_or_email() do
      %User{} = user ->
        add_and_flash(socket, company, user, role)
  ...
  def handle_event("change_role", %{"role_id" => role_id, "role" => role}, socket)
    ...
    case Companies.get_role(socket.assigns.company, role_id) do
      nil ->
        {:noreply, socket}

      role_row ->
        socket.assigns.current_user
        |> then(&Companies.update_role(role_row, role, &1))
        |> case do
          {:ok, _} ->
  ...
  def handle_event("remove", %{"id" => role_id}, socket) do
    case Companies.get_role(socket.assigns.company, role_id) do
      ...
      role_row ->
        case Companies.remove_role(role_row) do
          {:ok, _} ->
  ...
  defp add_and_flash(socket, company, user, role) do
    case Companies.add_role(company, user, role, socket.assigns.current_user) do
      {:ok, _} ->
        {:noreply,
         socket
         |> assign(:identifier, "")
  ...
  defp last_owner_error,
    do: gettext("A company must keep at least one owner, so the last owner cannot be changed.")
  ...
  def render(assigns) do
    ...
    <div class="mx-auto max-w-2xl py-6">
      <.manage_header company={@company} active={:roles} owner?={true} />
            ...
            <button
              ...
              data-confirm={
                if(role.user_id == @current_user_id,
                  do: gettext("Leave this company team?"),
