defmodule VutuvWeb.CompanyLive.Domains do
  @moduledoc """
  The owner-only multi-domain management page (`/companies/:slug/domains`, issue
  ...
  badge. Every active company keeps ≥ 1 verified domain; removing the primary
  ...
  import VutuvWeb.CompanyComponents, only: [manage_header: 1]
  ...
  def mount(_params, session, socket) do
    ...
    {:ok,
     ...
     |> assign(:page_title, gettext("Domains – %{name}", name: company.name))
     |> assign(:new_domain, "")
  ...
  defp load_domains(socket) do
    domains = Companies.list_domains(socket.assigns.company)
  ...
  def handle_event("add_domain", %{"domain" => domain} = params, socket) do
    ...
    case Companies.add_domain(company, domain, method) do
      {:ok, _domain} ->
        {:noreply,
         socket
         |> assign(:new_domain, "")
      ...
      {:error, :domain_taken} ->
        {:noreply,
         put_flash(socket, :error, gettext("That domain already belongs to a company page."))}
  ...
  def handle_event("set_method", %{"id" => id, "method" => method}, socket)
    ...
    with domain when not is_nil(domain) <- Companies.get_domain(socket.assigns.company, id),
         true <- is_nil(domain.verified_at) do
      {:ok, _} = Companies.set_domain_method(domain, method)
  ...
  def handle_event("verify", %{"id" => id}, socket) do
    ...
    case Companies.get_domain(company, id) do
      nil ->
        {:noreply, socket}
  ...
  def handle_event("set_primary", %{"id" => id}, socket) do
    with domain when not is_nil(domain) <- Companies.get_domain(socket.assigns.company, id),
         {:ok, _} <- Companies.set_primary_domain(socket.assigns.company, domain) do
  ...
  def handle_event("remove", %{"id" => id}, socket) do
    case Companies.get_domain(socket.assigns.company, id) do
      nil ->
        {:noreply, socket}

      domain ->
        case Companies.remove_domain(socket.assigns.company, domain) do
          {:ok, _} ->
          ...
          {:error, :last_domain} ->
            {:noreply,
             put_flash(
               ...
               :error,
               gettext(
                 "A company keeps at least one verified domain, so this one cannot be removed."
               )
             )}
        end
    end
  ...
  def render(assigns) do
    ...
    <div class="mx-auto max-w-2xl py-6">
      <.manage_header company={@company} active={:domains} owner?={true} />

      <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">{gettext("Domains")}</h1>
      <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">
        {gettext("A company can prove several domains. The primary one shows in the “Verified via …” badge.")}
      </p>
  ...
  defp verify_block(assigns, domain) do
    assigns =
      ...
      |> assign(:dns_value, Companies.dns_txt_value(domain))
      |> assign(:well_known_url, Companies.well_known_url(domain))
      |> assign(:well_known_content, Companies.well_known_content(domain))
