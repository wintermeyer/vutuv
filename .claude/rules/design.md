...
- `<.empty_add href={~p"/…/new"}>{gettext("Add …")}</.empty_add>` — **Track 2** owner empty-state CTA for profile sections: a full-width dashed-outline tile (plus glyph + label, brand-tint hover) that **replaces** the dead `<p class="text-slate-400">{gettext("Nothing here yet.")}</p>` line on the owner's own still-empty card, so a non-technical owner sees an obvious place to click instead of having to discover the quiet ⋯ menu. Points at the **same new-entry route** the card's `<.card_menu>` "Add entry" item uses; the label is a clear per-section call to action (`gettext("Add work experience")`, `Add a link`, `Write your first post` → `/feed`, General Info → `/edit`, …). Empty owner cards are already owner-only (the card's own `:if` hides an empty section from visitors), so just keep the `:if={collection == []}` empty guard at the call site — no extra `same_user?` needed. The Skills card is the exception: it keeps its inline quick-add `<.form>` (tags input) instead. Carries a `data-empty-add` hook for tests.
