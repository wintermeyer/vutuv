...
- `<.icon_repost class=… />` / `<.icon_bookmark filled?={…} class=… />` — the two Track 2 outline SVG icons used in more than one module (post card "Reposted by" line + action bar; shell + action bar). Size via `class` (defaults `h-5 w-5`); `filled?` switches the bookmark to solid. Icons used by a single LiveView stay private to it by convention.
