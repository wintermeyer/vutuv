...
- **Link preview card** (profile Links section, main column): `grid gap-4 sm:grid-cols-2`, each cell an `<a>` wrapping `<img src={Vutuv.Screenshot.url({url.screenshot, url}, :thumb)} width="400" height="264" loading="lazy" class="aspect-[400/264] w-full object-cover">` inside `overflow-hidden rounded-lg ring-1 ring-slate-200 dark:ring-slate-800`, then a `min-w-0` title/URL row (`truncate`) with the owner `<.row_edit_link>`. The screenshots come from `Vutuv.PageScreenshot` (headless Chromium, on link create/update); `Screenshot.url/2` falls back to `/images/screenshot.png`.
...
- `<.section_header title={…} add_href={owner? && ~p"/…/new"} />` — card header row: section title + optional right-aligned actions (falsy `add_href` hides the "Add" link); the `:action` slot adds a custom action, and multiple actions sit together in one right-aligned flex group.
- `<.row_edit_link edit_to={owner? && ~p"/…/edit"} title=… class=… />` — **Track 2** per-row pencil icon link, the owner's "this entry is yours" affordance on profile section rows (always visible, muted slate → brand on hover; renders nothing when `edit_to` is falsy, so the owner guard lives inline at the call site). Deletion intentionally does **not** sit on the row — every owned-resource edit form carries it via `<.form_actions delete_to={…} />`. Sections without per-entry edit (tags) put the pencil in the `<.section_header>` `:action` slot, pointing at the management page.
