...
- **A hook must never read the server's echo of its own value as a command: a value comparison cannot tell "stale" from "new", so the server has to ANNOUNCE a re-seed with a token.** An editor, tag box or any other widget that pushes its value through `phx-change` gets that value re-rendered back at it, so a patch is the normal case — and the answer to an *older* change arrives while the widget already holds newer text. The rule "the rendered value differs from what I last pushed, so rebuild" then rebuilds on that late echo, which for a `contenteditable` means the prose snaps back and the caret lands at the end (and again milliseconds later, when the newer answer arrives). It is the second, independent cause of the same "jumping cursor" report, it is invisible on a fast link, and LiveView's element locking does **not** protect you — it delays the patch, it does not discard a superseded one. So carry a token the server bumps only where it means it (`data-mde-seed`, bumped by `Composer.reset_composer/1` and `MessageLive.Index.assign_form/1`), re-seed only when the token changes **and** the value really differs (a rejoin restarts the counter at 0), and put the widget's own text back into the hidden form field on every ignored echo — morphdom overwrites that field, and a submit sends what it holds. Reproduce with `liveSocket.enableLatencySim(1500)` and one keystroke every 1.5s, not by typing fast. The tag box (`assets/js/tag_input.js`) still answers the same question the weaker way, by remembering the last 30 values it sent and consuming a match — that has no late-echo bug (a stale echo is in the list) but the mirror-image one its own comment records: a genuine server reset to a value the member once typed reads as an echo and is ignored. It shares `reset_composer/1` with the editor, so the token it needs already exists; give it one rather than growing a third variant.

- **A `push_event` answered by a hook whose element the same patch removes is never heard.** LiveView applies the patch first, which destroys the hook, and dispatches the pushed events afterwards, so a `handleEvent` inside that hook never runs and nothing logs it. Act in the click handler instead, or listen on `window` (`phx:<name>`), where LiveView dispatches every pushed event too. The ad card's ✕ writes its day cookie on the click for exactly this reason.
