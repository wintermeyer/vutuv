...
- **Build a test's *dates* from `Vutuv.BerlinTime.today()`, never `Date.utc_today()`** (and Berlin clock helpers, not `DateTime.utc_now()`, wherever a calendar day is the thing under test). vutuv stamps the Berlin calendar day, not the UTC day, into `published_on`, an ad's `day`, the age display and the today/yesterday post wording. A test that creates such a record and then builds its expected date or a `{from, to}` period from `Date.utc_today()` diverges from the code for the ~2 hours each night when Berlin is already the next day (≈22:00–24:00 UTC in CEST summer), so it passes all day and then fails deterministically only in that window — a wall-clock flake that reaches CI at the worst time. This exact gap crash-failed `posts_test.exs` author-posts period scoping until v7.55.1. **And place a fixture INSIDE the day the test judges** (`Vutuv.PostsHelpers.place_post_on_day!/3`, which anchors at that day's midday), never as `N * @day + offset` counted back from now: the offset comes off the current *time of day*, so a batch spread over an hour walks its tail into the day before for the first hour after local midnight. Eight feed-calendar tests went red at 00:06 and would have gone green again at 01:00 on their own, which reads as flakiness and is arithmetic.
- **A new regression test that stays green with *and* without its fix measures nothing — take the code back out rather than keep the test.** Calibrating (revert the fix via a patch you re-apply, watch it go red) already tells you to distrust a test that never fails; the missing half is what to do when it refuses to. The answer is almost never "keep both": an uncalibrated test is a claim nobody checks, and the code under it is untested weight in whatever it sits in. Ask first *why* it stays green — usually something else already guarantees the behaviour, and then the new code is redundant, not merely untested. That is how issue #1673 shipped without its tag-source exclusions: two correlated `EXISTS` subqueries in the hot feed query, guarding against one post reaching the reader twice, which both dedupe layers (`collapse_reposts/1` within a page, the feed LiveView's `shown_post_ids` across pages) already answer by post id. Keep the test if it pins a promise worth pinning, and say in a comment which layer actually enforces it, so nobody reads it as cover for the code you removed.
